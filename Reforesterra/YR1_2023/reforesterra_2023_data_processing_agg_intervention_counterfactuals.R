...
## Look-up table to translate sample names (only one CF and one I)
lutSampleNames <- read.csv("lut_sample_codes_v2.csv")

# We don't distinguish counterfactual types; we have only one.
# The site names are renamed for this purpose.
lookupSitenames2023 <- read.csv("lookupSitename2023_v2.csv", strip.white = T)
...
# rename the sample codes if necessary to have only one CF and one I type
rterra2023Raw$sample <- rterra2023Raw %>% dplyr::select(sample) %>% left_join(., y=  lutSampleNames, by = c("sample" = "old.sample.code")) %>% dplyr::select(new.sample.code) %>% pull(var = 1)
