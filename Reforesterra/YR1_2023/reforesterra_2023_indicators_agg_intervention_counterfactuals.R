...
source("reforesterra_2023_data_processing_agg_intervention_counterfactuals.R")
...
if( flag_save )
  ggsave("IMAGES/RTerra_raw_spec_replicates.png", width = 6, height = 4, dpi=300)
...
if(flag_save)
  ggsave("IMAGES/RTerra_raw_spec_repl-by-farm.png", width = 7, height=4, dpi = 300)
...
if(flag_save)
  ggsave("IMAGES/RTerra_raw_spec_group.png", width=6, height = 4, dpi=300)
...
if(flag_save)
  ggsave("IMAGES/RTerra_ESR_replicates.png", width=6, height = 4, dpi=300)
...
if(flag_save)
  ggsave("IMAGES/RTerra_ESR_group.png", width=6, height = 4, dpi=300)
...
if(flag_save)
  ggsave("IMAGES/RTerra_2023_ait_replicates.png", width=7, height=5, dpi=300)
...
# Plot Aitchison distance by land cover by site
siteHeatmap

if(flag_save)
  ggsave("IMAGES/RTerra_2023_ait_paired_lc-site",
...
if(flag_save)
  ggsave("IMAGES/RTerra_2023_ait_lc.png",
         width=6, height=5, dpi=300)
...
# LM
# lut_pretty_names <- data.frame(old = c("CF", "I", "R"),
#                                new = c("Counterfactual",
#                                        "Intervention",
#                                        "Forest"))
#rownames(compType2023) <- lut_pretty_names[match(rownames(compType2023), lut_pretty_names[,1]), 2]
...
typeHeatmap <-
    aitchisonLC2023 %>% aitHeatmap(fillColor1 = supportingColorPalette[2],
                                     fillColor2 = corporateColorPalette[4]) + scale_x_discrete(labels = c("Intervention", "Forest")) +
  scale_y_discrete(labels = c("Intervention", "Counterfactual"))

typeHeatmap

ggsave("RTerra_2023_dist_heatmap_2023.png",
       plot = typeHeatmap,
       device = "png",
       path = "IMAGES/",
       width = 8,
